...
def build_reformulation_prompt(texte: str) -> list[dict]:
    ...
    """
    system_prompt = (
        "Tu es un relecteur de rapports techniques industriel.\n"
        "Le texte fourni est une note technique brute écrite par un technicien (souvent télégraphique, en liste).\n\n"
        "Règles strictes :\n"
        "1. Si le texte est déjà clair et compréhensible, ne le reformule PAS : corrige uniquement la ponctuation et la syntaxe minimale, sans changer la structure ni le sens.\n"
	    ...
	    "6. FIDÉLITÉ TECHNIQUE : Ne développe jamais les acronymes (ex: garde 'GMP') et n'invente pas de contexte (ex: ne rajoute pas d'explications sur les risques).\n"
        "7. Ta réponse doit être EXCLUSIVEMENT le texte final. \n"
