...
@pytest.mark.asyncio
async def test_correction_returns_200(client: AsyncClient, mock_ollama_sauvegarder):
    mock_ollama_sauvegarder.return_value = "Texte corrigé."
    response = await client.post("/api/corriger", json={"texte": "Texte a corriger."})
    assert response.status_code == 200

@pytest.mark.asyncio
async def test_correction_response_contains_corrected_text(client: AsyncClient, mock_ollama_sauvegarder):
    mock_ollama_sauvegarder.return_value = "Texte corrigé."
    response = await client.post("/api/corriger", json={"texte": "Texte a corriger."})
    data = response.json()
...
@pytest.mark.asyncio
async def test_correction_response_contains_model_name(client: AsyncClient, mock_ollama_sauvegarder):
    mock_ollama_sauvegarder.return_value = "Texte corrigé."
    response = await client.post("/api/corriger", json={"texte": "Texte a corriger."})
    data = response.json()
...
@pytest.mark.asyncio
async def test_correction_calls_ollama_once(client: AsyncClient, mock_ollama_sauvegarder):
    mock_ollama_sauvegarder.return_value = "Texte corrigé."
...
# Ollama error cases
@pytest.mark.asyncio
async def test_correction_ollama_timeout_returns_504(client: AsyncClient, mock_ollama_sauvegarder):
...
@pytest.mark.asyncio
async def test_correction_ollama_unavailable_returns_503(client: AsyncClient, mock_ollama_sauvegarder):
    mock_ollama_sauvegarder.side_effect = HTTPException(status_code=503, detail="Unavailable")
    response = await client.post("/api/corriger", json={"texte": "test"})
    assert response.status_code == 503

@pytest.mark.asyncio
async def test_correction_empty_llm_response_returns_502(client: AsyncClient, mock_ollama_sauvegarder):
    mock_ollama_sauvegarder.return_value = ""
    response = await client.post("/api/corriger", json={"texte": "test"})
    assert response.status_code == 502

@pytest.mark.asyncio
async def test_correction_whitespace_llm_response_returns_502(client: AsyncClient, mock_ollama_sauvegarder):
    mock_ollama_sauvegarder.return_value = "   "
    response = await client.post("/api/corriger", json={"texte": "test"})
    assert response.status_code == 502
